...
function SeePicNew(f, imgdid, frname, hpos, acname)
{
	...
	if(!newobj)
	{
		...
		newobj.innerHTML = '<img src="data/images/loadinglit.gif" width="16" height="16" alit="" />上传中...';
	}
	newobj.style.display = 'block';
	//提交后还原form的action等参数
	nForm.action = acname;
...
function LoadQuickDiv(e, surl, oname, w, h)
{
	...
	newobj.innerHTML = '<div style="margin-top:10px;margin-left:10px;"><img src="data/images/loadinglit.gif" /> Loading...</div>';
	newobj.style.display = 'block';
	var myajax = new DedeAjax(newobj);
	myajax.SendGet(surl);
	fixupPos = false;
